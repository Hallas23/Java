...
public class Controller {
	...
	public PN opretPNOrdination(LocalDate startDen, LocalDate slutDen, Patient patient, Laegemiddel laegemiddel,
		...
		if (startDen.isAfter(slutDen))
			throw new IllegalArgumentException("Din slut dato skal være efter din start dato");

		PN pn = new PN(startDen, slutDen, antal);
		pn.setLægemiddel(laegemiddel);
	...
	public DagligFast opretDagligFastOrdination(LocalDate startDen, LocalDate slutDen, Patient patient,
		...
		df.setLægemiddel(laegemiddel);
		patient.addOrdination(df);
		df.opretDosis(LocalTime.of(8, 00), morgenAntal);
		df.opretDosis(LocalTime.of(12, 00), middagAntal);
		df.opretDosis(LocalTime.of(18, 00), aftenAntal);
		df.opretDosis(LocalTime.of(00, 00), natAntal);
		return df;
	}
	...
	public DagligSkaev opretDagligSkaevOrdination(LocalDate startDen, LocalDate slutDen, Patient patient,
		...
		if (klokkeSlet.length != antalEnheder.length) {
			throw new IllegalArgumentException("Dine to lister skal være lige lange");
		}
	...
	public void ordinationPNAnvendt(PN ordination, LocalDate dato) {
		...
		if (!gyldig)
			throw new IllegalArgumentException("Datoen er ude fra gyldighedsperioden");
	}
	...
	public void createSomeObjects() {
		...
		opretDagligFastOrdination(LocalDate.of(2019, 1, 1), LocalDate.of(2019, 1, 30),
				storage.getAllPatienter().get(1), storage.getAllLaegemidler().get(1), 0.1, 0.1, 2, 2);
